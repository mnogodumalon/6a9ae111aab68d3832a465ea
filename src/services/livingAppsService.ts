...
import type { Mitarbeiter, Werkzeuge, Ausleihen, Wartungen, Schaeden, CreateMitarbeiter, CreateWerkzeuge, CreateAusleihen, CreateWartungen, CreateSchaeden } from '@/types/app';
...
export class LivingAppsService {
  ...
  static async queryWerkzeuge(q: RecordQuery = {}): Promise<Werkzeuge[]> {
    const data = await callApi('GET', `/apps/${APP_IDS.WERKZEUGE}/records${recordQueryString(q)}`, undefined, { signal: q.signal });
    const records = Object.entries(data).map(([id, rec]: [string, any]) => ({
      record_id: id, ...rec,
      createdat: rec.created_at ?? '', updatedat: rec.updated_at ?? null,
    })) as Werkzeuge[];
  ...
  static async countWerkzeuge(filter?: string, signal?: AbortSignal): Promise<number> {
    const data = await callApi('GET', `/apps/${APP_IDS.WERKZEUGE}/aggregate_records${recordQueryString({ filter })}${filter ? '&' : '?'}value=count()`, undefined, { signal, silent: true });
    return parseAggregateCount(data);
  }
  ...
  // --- AUSLEIHEN ---
  static async getAusleihen(): Promise<Ausleihen[]> {
    const data = await callApi('GET', `/apps/${APP_IDS.AUSLEIHEN}/records`);
    const records = Object.entries(data).map(([id, rec]: [string, any]) => ({
      record_id: id, ...rec,
      createdat: rec.created_at ?? '', updatedat: rec.updated_at ?? null,
    })) as Ausleihen[];
    return hydrateRecords(records, 'ausleihen');
  }
  static async queryAusleihen(q: RecordQuery = {}): Promise<Ausleihen[]> {
    const data = await callApi('GET', `/apps/${APP_IDS.AUSLEIHEN}/records${recordQueryString(q)}`, undefined, { signal: q.signal });
    const records = Object.entries(data).map(([id, rec]: [string, any]) => ({
      record_id: id, ...rec,
      createdat: rec.created_at ?? '', updatedat: rec.updated_at ?? null,
    })) as Ausleihen[];
  ...
  static async countAusleihen(filter?: string, signal?: AbortSignal): Promise<number> {
    const data = await callApi('GET', `/apps/${APP_IDS.AUSLEIHEN}/aggregate_records${recordQueryString({ filter })}${filter ? '&' : '?'}value=count()`, undefined, { signal, silent: true });
    return parseAggregateCount(data);
  }
  static async getAusleihenEntry(id: string): Promise<Ausleihen | undefined> {
    const data = await callApi('GET', `/apps/${APP_IDS.AUSLEIHEN}/records/${id}`);
    const record = { record_id: data.id, ...data, createdat: data.created_at ?? '', updatedat: data.updated_at ?? null } as Ausleihen;
    return hydrateRecords([record], 'ausleihen')[0];
  }
  static async createAusleihenEntry(fields: CreateAusleihen): Promise<MutationResult> {
    const data = await callApi('POST', `/apps/${APP_IDS.AUSLEIHEN}/records`, { fields: cleanFieldsForApi(fields as any, 'ausleihen') });
    return { ...data, record_id: data.id };
  }
  static async updateAusleihenEntry(id: string, fields: Partial<CreateAusleihen>): Promise<MutationResult> {
    const data = await callApi('PATCH', `/apps/${APP_IDS.AUSLEIHEN}/records/${id}`, { fields: cleanFieldsForApi(fields as any, 'ausleihen') });
    return { ...data, record_id: data.id };
  }
  static async deleteAusleihenEntry(id: string) {
    return callApi('DELETE', `/apps/${APP_IDS.AUSLEIHEN}/records/${id}`);
  }

  // --- WARTUNGEN ---
  static async getWartungen(): Promise<Wartungen[]> {
    const data = await callApi('GET', `/apps/${APP_IDS.WARTUNGEN}/records`);
    const records = Object.entries(data).map(([id, rec]: [string, any]) => ({
      record_id: id, ...rec,
      createdat: rec.created_at ?? '', updatedat: rec.updated_at ?? null,
    })) as Wartungen[];
    return hydrateRecords(records, 'wartungen');
  }
  static async queryWartungen(q: RecordQuery = {}): Promise<Wartungen[]> {
    const data = await callApi('GET', `/apps/${APP_IDS.WARTUNGEN}/records${recordQueryString(q)}`, undefined, { signal: q.signal });
    const records = Object.entries(data).map(([id, rec]: [string, any]) => ({
      record_id: id, ...rec,
      createdat: rec.created_at ?? '', updatedat: rec.updated_at ?? null,
    })) as Wartungen[];
  ...
  static async countWartungen(filter?: string, signal?: AbortSignal): Promise<number> {
    const data = await callApi('GET', `/apps/${APP_IDS.WARTUNGEN}/aggregate_records${recordQueryString({ filter })}${filter ? '&' : '?'}value=count()`, undefined, { signal, silent: true });
    return parseAggregateCount(data);
  }
  static async getWartungenEntry(id: string): Promise<Wartungen | undefined> {
    const data = await callApi('GET', `/apps/${APP_IDS.WARTUNGEN}/records/${id}`);
    const record = { record_id: data.id, ...data, createdat: data.created_at ?? '', updatedat: data.updated_at ?? null } as Wartungen;
    return hydrateRecords([record], 'wartungen')[0];
  }
  static async createWartungenEntry(fields: CreateWartungen): Promise<MutationResult> {
    const data = await callApi('POST', `/apps/${APP_IDS.WARTUNGEN}/records`, { fields: cleanFieldsForApi(fields as any, 'wartungen') });
    return { ...data, record_id: data.id };
  }
  static async updateWartungenEntry(id: string, fields: Partial<CreateWartungen>): Promise<MutationResult> {
    const data = await callApi('PATCH', `/apps/${APP_IDS.WARTUNGEN}/records/${id}`, { fields: cleanFieldsForApi(fields as any, 'wartungen') });
    return { ...data, record_id: data.id };
  }
  static async deleteWartungenEntry(id: string) {
    return callApi('DELETE', `/apps/${APP_IDS.WARTUNGEN}/records/${id}`);
  }

  // --- SCHAEDEN ---
  static async getSchaeden(): Promise<Schaeden[]> {
    const data = await callApi('GET', `/apps/${APP_IDS.SCHAEDEN}/records`);
    const records = Object.entries(data).map(([id, rec]: [string, any]) => ({
      record_id: id, ...rec,
      createdat: rec.created_at ?? '', updatedat: rec.updated_at ?? null,
    })) as Schaeden[];
    return hydrateRecords(records, 'schaeden');
  }
  static async querySchaeden(q: RecordQuery = {}): Promise<Schaeden[]> {
    const data = await callApi('GET', `/apps/${APP_IDS.SCHAEDEN}/records${recordQueryString(q)}`, undefined, { signal: q.signal });
    const records = Object.entries(data).map(([id, rec]: [string, any]) => ({
      record_id: id, ...rec,
      createdat: rec.created_at ?? '', updatedat: rec.updated_at ?? null,
    })) as Schaeden[];
  ...
  static async countSchaeden(filter?: string, signal?: AbortSignal): Promise<number> {
    const data = await callApi('GET', `/apps/${APP_IDS.SCHAEDEN}/aggregate_records${recordQueryString({ filter })}${filter ? '&' : '?'}value=count()`, undefined, { signal, silent: true });
    return parseAggregateCount(data);
  }
  static async getSchaedenEntry(id: string): Promise<Schaeden | undefined> {
    const data = await callApi('GET', `/apps/${APP_IDS.SCHAEDEN}/records/${id}`);
    const record = { record_id: data.id, ...data, createdat: data.created_at ?? '', updatedat: data.updated_at ?? null } as Schaeden;
    return hydrateRecords([record], 'schaeden')[0];
  }
  static async createSchaedenEntry(fields: CreateSchaeden): Promise<MutationResult> {
    const data = await callApi('POST', `/apps/${APP_IDS.SCHAEDEN}/records`, { fields: cleanFieldsForApi(fields as any, 'schaeden') });
    return { ...data, record_id: data.id };
  }
  static async updateSchaedenEntry(id: string, fields: Partial<CreateSchaeden>): Promise<MutationResult> {
    const data = await callApi('PATCH', `/apps/${APP_IDS.SCHAEDEN}/records/${id}`, { fields: cleanFieldsForApi(fields as any, 'schaeden') });
    return { ...data, record_id: data.id };
  }
  static async deleteSchaedenEntry(id: string) {
    return callApi('DELETE', `/apps/${APP_IDS.SCHAEDEN}/records/${id}`);
  }
