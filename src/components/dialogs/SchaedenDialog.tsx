/**
 * SchaedenDialog — pre-generated create/edit dialog for Schaeden.
 *
 ...
 * SchaedenDialogDefaults — NOT the entity field type: lookup fields accept
 * the bare KEY string (or LookupValue), applookup fields the bare record id
 * (or record URL); the dialog normalizes. Type prefill STATE with the export:
 *  ❌ useState<Partial<Schaeden['fields']>>({ … })   // LookupValue fields reject string prefills (TS2322)
 *  ✓ useState<SchaedenDialogDefaults | undefined>(undefined)
 */
...
import { formEnhancements, computedDeps, computedApplookupRefs } from '@/config/form-enhancements/Schaeden';
...
/** Widened prefill type for SchaedenDialog.defaultValues — see file header. */
export type SchaedenDialogDefaults = Schaeden['fields'];

interface SchaedenDialogProps {
  open: boolean;
  onClose: () => void;
  onSubmit: (fields: Schaeden['fields']) => Promise<void>;
  /** SHAPE-TOLERANT: lookup fields accept the bare key (string) or the
   *  LookupValue object; applookup fields the bare record id or the full
   *  record URL — the dialog normalizes both. */
  defaultValues?: SchaedenDialogDefaults;
...
export function SchaedenDialog({ open, onClose, onSubmit, defaultValues, recordId, werkzeugeList, enablePhotoScan = true, enablePhotoLocation = true }: SchaedenDialogProps) {
  const [fields, setFields] = useState<Partial<Schaeden['fields']>>({});
  ...
  const REQUIRED_FIELDS = ['werkzeug', 'gemeldet_am'] as const;
  ...
  useEffect(() => {
    if (open) {
      setFields(applyDefaults(normalizedDefaults ?? {}, formEnhancements.defaults) as Partial<Schaeden['fields']>);
      setPreview(null);
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      const clean = cleanFieldsForApi(merged, 'schaeden');
      await onSubmit(clean as Schaeden['fields']);
      onClose();
    } catch (err) {
  ...
  async function handleAiExtract(file?: File) {
    ...
    try {
      ...
      setFields(prev => {
        ...
        return merged as Partial<Schaeden['fields']>;
      });
  ...
  const DIALOG_INTENT = defaultValues
    ? t('edit_entity', { entity: appLabel('schaeden') })
    : t('new_entity', { entity: appLabel('schaeden') });

  const fieldBlocks: Record<string, React.ReactNode> = {
    'werkzeug': (
      <div key="werkzeug" className="space-y-1.5">
        <Label htmlFor="werkzeug">{fieldLabel('schaeden', 'werkzeug')} <span className="text-destructive" aria-hidden="true">*</span></Label>
        <Combobox
          id="werkzeug"
          placeholder="Welches Werkzeug ist beschädigt?"
        ...
        {showErrors && !fields.werkzeug && (
          <p className="text-xs text-destructive mt-1" role="alert">{requiredMessage('schaeden', 'werkzeug')}</p>
        )}
      </div>
    ),
    'gemeldet_am': (
      <div key="gemeldet_am" className="space-y-1.5">
        <Label htmlFor="gemeldet_am">{fieldLabel('schaeden', 'gemeldet_am')} <span className="text-destructive" aria-hidden="true">*</span></Label>
        <DatePicker
          id="gemeldet_am"
          placeholder="Wann wurde der Schaden gemeldet?"
          mode="date"
          value={fields.gemeldet_am ?? null}
          onChange={v => setFields(f => ({ ...f, gemeldet_am: v ?? undefined }))}
          required
        />
        {showErrors && !fields.gemeldet_am && (
          <p className="text-xs text-destructive mt-1" role="alert">{requiredMessage('schaeden', 'gemeldet_am')}</p>
        )}
      </div>
    ),
    'beschreibung': (
      <div key="beschreibung" className="space-y-1.5">
        <Label htmlFor="beschreibung">{fieldLabel('schaeden', 'beschreibung')}</Label>
        <Textarea
          id="beschreibung"
          placeholder="Beschreibung des Schadens, Ursache, Schweregrad..."
    ...
    'foto': (
      <div key="foto" className="space-y-1.5">
        <Label htmlFor="foto">{fieldLabel('schaeden', 'foto')}</Label>
        {fields.foto ? (
  ...
  const FIELD_LABELS: Record<string, string> = {"werkzeug": "Werkzeug", "gemeldet_am": "Gemeldet am", "beschreibung": "Beschreibung", "foto": "Foto"};
  const CURRENCY_KEYS = new Set<string>([]);
  // Applookup-Referenz-Labels: pro applookup-Feld in dieser Form (ownKey)
  // eine Map { lookupKey: label } für ALLE Felder des Target-Schemas. Wird
  // beim Render-Walk gefiltert auf die in der computed-Formel tatsächlich
  // referenzierten lookupKeys (siehe applookupRefs unten).
  const APPLOOKUP_LABELS: Record<string, Record<string, string>> = {"werkzeug": {"bezeichnung": "Bezeichnung", "inventarnummer": "Inventarnummer", "status": "Status", "bestand": "Bestand", "standort": "Standort", "anschaffungspreis": "Anschaffungspreis (Euro)", "handbuch": "Handbuch", "kategorien": "Kategorien", "ersatz_fuer": "Ersatz für"}};
          ...
          <div className="flex-1 overflow-y-auto overflow-x-hidden px-6 py-4 space-y-4 min-w-0">
            ...
            {recordId && (
              <div className="pt-2 border-t border-border">
                <AttachmentsSection appId={APP_IDS.SCHAEDEN} recordId={recordId} />
              </div>
            )}
          </div>
          {submitError && (
