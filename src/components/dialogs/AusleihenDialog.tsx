/**
 * AusleihenDialog — pre-generated create/edit dialog for Ausleihen.
 *
 ...
 * AusleihenDialogDefaults — NOT the entity field type: lookup fields accept
 * the bare KEY string (or LookupValue), applookup fields the bare record id
 * (or record URL); the dialog normalizes. Type prefill STATE with the export:
 *  ❌ useState<Partial<Ausleihen['fields']>>({ … })   // LookupValue fields reject string prefills (TS2322)
 *  ✓ useState<AusleihenDialogDefaults | undefined>(undefined)
 */
import { useState, useEffect, useMemo, useRef, useCallback } from 'react';
import type { Ausleihen, Werkzeuge, Mitarbeiter, LookupValue } from '@/types/app';
...
import { formEnhancements, computedDeps, computedApplookupRefs } from '@/config/form-enhancements/Ausleihen';
...
/** Widened prefill type for AusleihenDialog.defaultValues — see file header. */
export type AusleihenDialogDefaults = Omit<Ausleihen['fields'], 'zustand_bei_rueckgabe'> & {
    zustand_bei_rueckgabe?: LookupValue | string;
  };

interface AusleihenDialogProps {
  open: boolean;
  onClose: () => void;
  onSubmit: (fields: Ausleihen['fields']) => Promise<void>;
  /** SHAPE-TOLERANT: lookup fields accept the bare key (string) or the
   *  LookupValue object; applookup fields the bare record id or the full
   *  record URL — the dialog normalizes both. */
  defaultValues?: AusleihenDialogDefaults;
...
const NORMALIZE_LOOKUPS: Record<string, readonly { key: string; label: string }[]> = {
  zustand_bei_rueckgabe: LOOKUP_OPTIONS['ausleihen']?.['zustand_bei_rueckgabe'] ?? [],
};
const NORMALIZE_APPLOOKUPS: Record<string, string> = {
  werkzeug: APP_IDS.WERKZEUGE,
  mitarbeiter: APP_IDS.MITARBEITER,
};
...
export function AusleihenDialog({ open, onClose, onSubmit, defaultValues, recordId, werkzeugeList, mitarbeiterList, enablePhotoScan = true, enablePhotoLocation = true }: AusleihenDialogProps) {
  const [fields, setFields] = useState<Partial<Ausleihen['fields']>>({});
  ...
  const REQUIRED_FIELDS = ['werkzeug', 'mitarbeiter', 'ausgabe'] as const;
  ...
  useEffect(() => {
    if (open) {
      setFields(applyDefaults(normalizedDefaults ?? {}, formEnhancements.defaults) as Partial<Ausleihen['fields']>);
      setPreview(null);
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      const clean = cleanFieldsForApi(merged, 'ausleihen');
      await onSubmit(clean as Ausleihen['fields']);
      onClose();
    } catch (err) {
  ...
  async function handleAiExtract(file?: File) {
    ...
    try {
      ...
      const schema = `{\n  "werkzeug": string | null, // Display name from Werkzeuge (see <available-records>)\n  "mitarbeiter": string | null, // Display name from Mitarbeiter (see <available-records>)\n  "ausgabe": string | null, // YYYY-MM-DDTHH:MM\n  "rueckgabe_geplant": string | null, // YYYY-MM-DD\n  "rueckgabe_erfolgt": string | null, // YYYY-MM-DDTHH:MM\n  "zustand_bei_rueckgabe": LookupValue | null, // Zustand bei Rückgabe (select one key: "einwandfrei" | "gebrauchsspuren" | "beschaedigt" | "verloren") mapping: einwandfrei=Einwandfrei, gebrauchsspuren=Gebrauchsspuren, beschaedigt=Beschädigt, verloren=Verloren\n}`;
      ...
      setFields(prev => {
        ...
        if (werkzeugName) {
          const werkzeugMatch = werkzeugeList.find(r => matchName(werkzeugName!, [String(r.fields.bezeichnung ?? '')]));
        ...
        return merged as Partial<Ausleihen['fields']>;
      });
      setAiText('');
      setScanSuccess(true);
  ...
  const DIALOG_INTENT = defaultValues
    ? t('edit_entity', { entity: appLabel('ausleihen') })
    : t('new_entity', { entity: appLabel('ausleihen') });

  const fieldBlocks: Record<string, React.ReactNode> = {
    'werkzeug': (
      <div key="werkzeug" className="space-y-1.5">
        <Label htmlFor="werkzeug">{fieldLabel('ausleihen', 'werkzeug')} <span className="text-destructive" aria-hidden="true">*</span></Label>
        <Combobox
          id="werkzeug"
          placeholder="Welches Werkzeug wird verliehen?"
        ...
        {showErrors && !fields.werkzeug && (
          <p className="text-xs text-destructive mt-1" role="alert">{requiredMessage('ausleihen', 'werkzeug')}</p>
        )}
      </div>
    ),
    'mitarbeiter': (
      <div key="mitarbeiter" className="space-y-1.5">
        <Label htmlFor="mitarbeiter">{fieldLabel('ausleihen', 'mitarbeiter')} <span className="text-destructive" aria-hidden="true">*</span></Label>
        <Combobox
          id="mitarbeiter"
          placeholder="Welcher Mitarbeiter leiht aus?"
          items={mitarbeiterListAll.map(r => ({
            id: r.record_id,
            label: String(r.fields.vorname ?? r.record_id),
          }))}
          value={extractRecordId(fields.mitarbeiter)}
          onChange={id => setFields(f => ({ ...f, mitarbeiter: id ? createRecordUrl(APP_IDS.MITARBEITER, id) : undefined }))}
          onCreateNew={(q) => openCreateMitarbeiter("mitarbeiter", q)}
          createLabel={t('create_in', { entity: appLabel('mitarbeiter') })}
        />
        {showErrors && !fields.mitarbeiter && (
          <p className="text-xs text-destructive mt-1" role="alert">{requiredMessage('ausleihen', 'mitarbeiter')}</p>
        )}
      </div>
    ),
    'ausgabe': (
      <div key="ausgabe" className="space-y-1.5">
        <Label htmlFor="ausgabe">{fieldLabel('ausleihen', 'ausgabe')} <span className="text-destructive" aria-hidden="true">*</span></Label>
        <DatePicker
          id="ausgabe"
          placeholder="Wann wird es ausgegeben?"
          mode="datetime"
          value={fields.ausgabe ?? null}
          onChange={v => setFields(f => ({ ...f, ausgabe: v ?? undefined }))}
          required
        />
        {showErrors && !fields.ausgabe && (
          <p className="text-xs text-destructive mt-1" role="alert">{requiredMessage('ausleihen', 'ausgabe')}</p>
        )}
      </div>
    ),
    'rueckgabe_geplant': (
      <div key="rueckgabe_geplant" className="space-y-1.5">
        <Label htmlFor="rueckgabe_geplant">{fieldLabel('ausleihen', 'rueckgabe_geplant')}</Label>
        <DatePicker
          id="rueckgabe_geplant"
          placeholder="Wann soll es zurück?"
          mode="date"
          value={fields.rueckgabe_geplant ?? null}
          onChange={v => setFields(f => ({ ...f, rueckgabe_geplant: v ?? undefined }))}
        />
      </div>
    ),
    'rueckgabe_erfolgt': (
      <div key="rueckgabe_erfolgt" className="space-y-1.5">
        <Label htmlFor="rueckgabe_erfolgt">{fieldLabel('ausleihen', 'rueckgabe_erfolgt')}</Label>
        <DatePicker
          id="rueckgabe_erfolgt"
          placeholder="Wann kommt es zurück?"
          mode="datetime"
          value={fields.rueckgabe_erfolgt ?? null}
          onChange={v => setFields(f => ({ ...f, rueckgabe_erfolgt: v ?? undefined }))}
        />
      </div>
    ),
    'zustand_bei_rueckgabe': (
      <div key="zustand_bei_rueckgabe" className="space-y-1.5">
        <Label htmlFor="zustand_bei_rueckgabe">{fieldLabel('ausleihen', 'zustand_bei_rueckgabe')}</Label>
        <div role="radiogroup" className="flex flex-wrap gap-1.5">
          <button
            type="button"
            role="radio"
            aria-checked={lookupKey(fields.zustand_bei_rueckgabe) === 'einwandfrei'}
            onClick={() => setFields(f => ({ ...f, zustand_bei_rueckgabe: (lookupKey(f.zustand_bei_rueckgabe) === 'einwandfrei' ? undefined : 'einwandfrei') as any }))}
            className={`inline-flex items-center justify-center min-h-9 max-sm:min-h-11 max-sm:px-4 rounded-full border px-3 py-1.5 text-sm font-medium transition-colors ${
              lookupKey(fields.zustand_bei_rueckgabe) === 'einwandfrei'
                ? 'bg-foreground text-background border-foreground'
                : 'bg-background text-foreground border-input hover:bg-accent'
            }`}
          >
            {lookupLabel('ausleihen', 'zustand_bei_rueckgabe', 'einwandfrei') ?? 'Einwandfrei'}
          </button>
          <button
            type="button"
            role="radio"
            aria-checked={lookupKey(fields.zustand_bei_rueckgabe) === 'gebrauchsspuren'}
            onClick={() => setFields(f => ({ ...f, zustand_bei_rueckgabe: (lookupKey(f.zustand_bei_rueckgabe) === 'gebrauchsspuren' ? undefined : 'gebrauchsspuren') as any }))}
            className={`inline-flex items-center justify-center min-h-9 max-sm:min-h-11 max-sm:px-4 rounded-full border px-3 py-1.5 text-sm font-medium transition-colors ${
              lookupKey(fields.zustand_bei_rueckgabe) === 'gebrauchsspuren'
                ? 'bg-foreground text-background border-foreground'
                : 'bg-background text-foreground border-input hover:bg-accent'
            }`}
          >
            {lookupLabel('ausleihen', 'zustand_bei_rueckgabe', 'gebrauchsspuren') ?? 'Gebrauchsspuren'}
          </button>
          <button
            type="button"
            role="radio"
            aria-checked={lookupKey(fields.zustand_bei_rueckgabe) === 'beschaedigt'}
            onClick={() => setFields(f => ({ ...f, zustand_bei_rueckgabe: (lookupKey(f.zustand_bei_rueckgabe) === 'beschaedigt' ? undefined : 'beschaedigt') as any }))}
            className={`inline-flex items-center justify-center min-h-9 max-sm:min-h-11 max-sm:px-4 rounded-full border px-3 py-1.5 text-sm font-medium transition-colors ${
              lookupKey(fields.zustand_bei_rueckgabe) === 'beschaedigt'
                ? 'bg-foreground text-background border-foreground'
                : 'bg-background text-foreground border-input hover:bg-accent'
            }`}
          >
            {lookupLabel('ausleihen', 'zustand_bei_rueckgabe', 'beschaedigt') ?? 'Beschädigt'}
          </button>
          <button
            type="button"
            role="radio"
            aria-checked={lookupKey(fields.zustand_bei_rueckgabe) === 'verloren'}
            onClick={() => setFields(f => ({ ...f, zustand_bei_rueckgabe: (lookupKey(f.zustand_bei_rueckgabe) === 'verloren' ? undefined : 'verloren') as any }))}
            className={`inline-flex items-center justify-center min-h-9 max-sm:min-h-11 max-sm:px-4 rounded-full border px-3 py-1.5 text-sm font-medium transition-colors ${
              lookupKey(fields.zustand_bei_rueckgabe) === 'verloren'
                ? 'bg-foreground text-background border-foreground'
                : 'bg-background text-foreground border-input hover:bg-accent'
            }`}
          >
            {lookupLabel('ausleihen', 'zustand_bei_rueckgabe', 'verloren') ?? 'Verloren'}
          </button>
        </div>
      </div>
    ),
  ...
  const FIELD_LABELS: Record<string, string> = {"werkzeug": "Werkzeug", "mitarbeiter": "Mitarbeiter", "ausgabe": "Ausgabe", "rueckgabe_geplant": "Rückgabe geplant", "rueckgabe_erfolgt": "Rückgabe erfolgt", "zustand_bei_rueckgabe": "Zustand bei Rückgabe"};
  const CURRENCY_KEYS = new Set<string>([]);
  // Applookup-Referenz-Labels: pro applookup-Feld in dieser Form (ownKey)
  // eine Map { lookupKey: label } für ALLE Felder des Target-Schemas. Wird
  // beim Render-Walk gefiltert auf die in der computed-Formel tatsächlich
  // referenzierten lookupKeys (siehe applookupRefs unten).
  const APPLOOKUP_LABELS: Record<string, Record<string, string>> = {"werkzeug": {"bezeichnung": "Bezeichnung", "inventarnummer": "Inventarnummer", "status": "Status", "bestand": "Bestand", "standort": "Standort", "anschaffungspreis": "Anschaffungspreis (Euro)", "handbuch": "Handbuch", "kategorien": "Kategorien", "ersatz_fuer": "Ersatz für"}, "mitarbeiter": {"vorname": "Vorname", "nachname": "Nachname", "personalnummer": "Personalnummer", "abteilung": "Abteilung", "mobil": "Mobil", "aktiv": "Aktiv"}};
          ...
          <div className="flex-1 overflow-y-auto overflow-x-hidden px-6 py-4 space-y-4 min-w-0">
            ...
            {recordId && (
              <div className="pt-2 border-t border-border">
                <AttachmentsSection appId={APP_IDS.AUSLEIHEN} recordId={recordId} />
              </div>
            )}
          </div>
          {submitError && (
    ...
    {createWerkzeugeOpen && (
      <WerkzeugeDialog
        ...
        defaultValues={createWerkzeugeInitial
          ? ({ bezeichnung: createWerkzeugeInitial } as any)
          : undefined}
