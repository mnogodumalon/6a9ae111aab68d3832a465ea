...
export type MitarbeiterDialogDefaults = Omit<Mitarbeiter['fields'], 'abteilung'> & {
...
const NORMALIZE_LOOKUPS: Record<string, readonly { key: string; label: string }[]> = {
  abteilung: LOOKUP_OPTIONS['mitarbeiter']?.['abteilung'] ?? [],
};
...
export function MitarbeiterDialog({ open, onClose, onSubmit, defaultValues, recordId, enablePhotoScan = true, enablePhotoLocation = true }: MitarbeiterDialogProps) {
  ...
  async function handleAiExtract(file?: File) {
    ...
    try {
      ...
      const schema = `{\n  "vorname": string | null, // Vorname\n  "nachname": string | null, // Nachname\n  "personalnummer": string | null, // Personalnummer\n  "abteilung": LookupValue | null, // Abteilung (select one key: "strassenbau" | "gruenpflege" | "hochbau" | "elektrik" | "fuhrpark" | "reinigung" | "verwaltung" | "winterdienst") mapping: strassenbau=Straßenbau, gruenpflege=Grünpflege, hochbau=Hochbau, elektrik=Elektrik, fuhrpark=Fuhrpark, reinigung=Reinigung, verwaltung=Verwaltung, winterdienst=Winterdienst\n  "mobil": string | null, // Mobil\n  "aktiv": boolean | null, // Aktiv\n}`;
  ...
  const fieldBlocks: Record<string, React.ReactNode> = {
    ...
    'personalnummer': (
      <div key="personalnummer" className="space-y-1.5">
        <Label htmlFor="personalnummer">{fieldLabel('mitarbeiter', 'personalnummer')}</Label>
        <Input
          id="personalnummer"
          placeholder="z. B. MA-2026-042"
    ...
    'abteilung': (
      <div key="abteilung" className="space-y-1.5">
        ...
        >
          <SelectTrigger id="abteilung" className="max-sm:h-11"><SelectValue placeholder="Aus 8 Abteilungen wählen" /></SelectTrigger>
          <SelectContent>
            <SelectItem value="none">—</SelectItem>
            <SelectItem value="strassenbau">{lookupLabel('mitarbeiter', 'abteilung', 'strassenbau') ?? 'Straßenbau'}</SelectItem>
            <SelectItem value="gruenpflege">{lookupLabel('mitarbeiter', 'abteilung', 'gruenpflege') ?? 'Grünpflege'}</SelectItem>
            <SelectItem value="hochbau">{lookupLabel('mitarbeiter', 'abteilung', 'hochbau') ?? 'Hochbau'}</SelectItem>
            <SelectItem value="elektrik">{lookupLabel('mitarbeiter', 'abteilung', 'elektrik') ?? 'Elektrik'}</SelectItem>
            <SelectItem value="fuhrpark">{lookupLabel('mitarbeiter', 'abteilung', 'fuhrpark') ?? 'Fuhrpark'}</SelectItem>
            <SelectItem value="reinigung">{lookupLabel('mitarbeiter', 'abteilung', 'reinigung') ?? 'Reinigung'}</SelectItem>
            <SelectItem value="verwaltung">{lookupLabel('mitarbeiter', 'abteilung', 'verwaltung') ?? 'Verwaltung'}</SelectItem>
            <SelectItem value="winterdienst">{lookupLabel('mitarbeiter', 'abteilung', 'winterdienst') ?? 'Winterdienst'}</SelectItem>
          </SelectContent>
        </Select>
      </div>
    ),
    'mobil': (
      <div key="mobil" className="space-y-1.5">
        <Label htmlFor="mobil">{fieldLabel('mitarbeiter', 'mobil')}</Label>
        <Input
          id="mobil"
          type="tel"
          inputMode="tel"
          placeholder="z. B. 0171 123 4567"
          value={fields.mobil ?? ''}
    ...
    'aktiv': (
      <div key="aktiv" className="space-y-1.5">
        <Label htmlFor="aktiv">{fieldLabel('mitarbeiter', 'aktiv')}</Label>
        <div className="flex items-center gap-2 pt-1">
          ...
          />
          <Label htmlFor="aktiv" className="font-normal">{fieldLabel('mitarbeiter', 'aktiv')}</Label>
        </div>
      </div>
    ),
  };
  ...
  const FIELD_LABELS: Record<string, string> = {"vorname": "Vorname", "nachname": "Nachname", "personalnummer": "Personalnummer", "abteilung": "Abteilung", "mobil": "Mobil", "aktiv": "Aktiv"};
