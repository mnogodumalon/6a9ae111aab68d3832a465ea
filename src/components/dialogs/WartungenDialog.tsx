/**
 * WartungenDialog — pre-generated create/edit dialog for Wartungen.
 *
 ...
 * WartungenDialogDefaults — NOT the entity field type: lookup fields accept
 * the bare KEY string (or LookupValue), applookup fields the bare record id
 * (or record URL); the dialog normalizes. Type prefill STATE with the export:
 *  ❌ useState<Partial<Wartungen['fields']>>({ … })   // LookupValue fields reject string prefills (TS2322)
 *  ✓ useState<WartungenDialogDefaults | undefined>(undefined)
 */
...
import { formEnhancements, computedDeps, computedApplookupRefs } from '@/config/form-enhancements/Wartungen';
...
/** Widened prefill type for WartungenDialog.defaultValues — see file header. */
export type WartungenDialogDefaults = Wartungen['fields'];

interface WartungenDialogProps {
  open: boolean;
  onClose: () => void;
  onSubmit: (fields: Wartungen['fields']) => Promise<void>;
  /** SHAPE-TOLERANT: lookup fields accept the bare key (string) or the
   *  LookupValue object; applookup fields the bare record id or the full
   *  record URL — the dialog normalizes both. */
  defaultValues?: WartungenDialogDefaults;
...
export function WartungenDialog({ open, onClose, onSubmit, defaultValues, recordId, werkzeugeList, enablePhotoScan = true, enablePhotoLocation = true }: WartungenDialogProps) {
  const [fields, setFields] = useState<Partial<Wartungen['fields']>>({});
  ...
  const REQUIRED_FIELDS = ['datum', 'werkzeug'] as const;
  ...
  useEffect(() => {
    if (open) {
      setFields(applyDefaults(normalizedDefaults ?? {}, formEnhancements.defaults) as Partial<Wartungen['fields']>);
      setPreview(null);
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      const clean = cleanFieldsForApi(merged, 'wartungen');
      await onSubmit(clean as Wartungen['fields']);
      onClose();
    } catch (err) {
  ...
  async function handleAiExtract(file?: File) {
    ...
    try {
      ...
      setFields(prev => {
        ...
        return merged as Partial<Wartungen['fields']>;
      });
      setAiText('');
      setScanSuccess(true);
  ...
  const DIALOG_INTENT = defaultValues
    ? t('edit_entity', { entity: appLabel('wartungen') })
    : t('new_entity', { entity: appLabel('wartungen') });

  const fieldBlocks: Record<string, React.ReactNode> = {
    'datum': (
      <div key="datum" className="space-y-1.5">
        <Label htmlFor="datum">{fieldLabel('wartungen', 'datum')} <span className="text-destructive" aria-hidden="true">*</span></Label>
        <DatePicker
          id="datum"
          placeholder="Wartungsdatum wählen"
          mode="date"
        ...
        {showErrors && !fields.datum && (
          <p className="text-xs text-destructive mt-1" role="alert">{requiredMessage('wartungen', 'datum')}</p>
        )}
      </div>
    ),
    'beschreibung': (
      <div key="beschreibung" className="space-y-1.5">
        <Label htmlFor="beschreibung">{fieldLabel('wartungen', 'beschreibung')}</Label>
        <Textarea
          id="beschreibung"
          placeholder="Was wurde gewartet oder repariert?"
    ...
    'kosten': (
      <div key="kosten" className="space-y-1.5">
        <Label htmlFor="kosten">{fieldLabel('wartungen', 'kosten')}</Label>
        <Input
          id="kosten"
          type="number"
          inputMode="decimal"
    ...
    'erledigt': (
      <div key="erledigt" className="space-y-1.5">
        <Label htmlFor="erledigt">{fieldLabel('wartungen', 'erledigt')}</Label>
        <div className="flex items-center gap-2 pt-1">
          ...
          />
          <Label htmlFor="erledigt" className="font-normal">{fieldLabel('wartungen', 'erledigt')}</Label>
        </div>
      </div>
    ),
    'werkzeug': (
      <div key="werkzeug" className="space-y-1.5">
        <Label htmlFor="werkzeug">{fieldLabel('wartungen', 'werkzeug')} <span className="text-destructive" aria-hidden="true">*</span></Label>
        <Combobox
          id="werkzeug"
          placeholder="Welches Werkzeug warten?"
        ...
        {showErrors && !fields.werkzeug && (
          <p className="text-xs text-destructive mt-1" role="alert">{requiredMessage('wartungen', 'werkzeug')}</p>
        )}
      </div>
    ),
  };
  ...
  const FIELD_LABELS: Record<string, string> = {"datum": "Datum", "beschreibung": "Beschreibung", "kosten": "Kosten (Euro)", "erledigt": "Erledigt", "werkzeug": "Werkzeug"};
          ...
          <div className="flex-1 overflow-y-auto overflow-x-hidden px-6 py-4 space-y-4 min-w-0">
            ...
            {recordId && (
              <div className="pt-2 border-t border-border">
                <AttachmentsSection appId={APP_IDS.WARTUNGEN} recordId={recordId} />
              </div>
            )}
          </div>
          {submitError && (
